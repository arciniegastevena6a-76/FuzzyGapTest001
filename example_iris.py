...
Modifications for scientific rigor:
- Removed fixed random seed to allow true random sampling
- Added multi-run capability for statistical analysis
- Output includes mean, std, and success rate across multiple runs
"""
...
def example_iris_incomplete_fod(random_seed: int = None, verbose: bool = True):
    """
    ...
    # Use provided seed or generate random one for data split
...
def example_unknown_class_only(random_seed: int = None, verbose: bool = True):
    """
    ...
    Returns:
        m_empty_mean: Mean empty set mass
        per_attr_means: Per-attribute mean empty set masses
    """
    if verbose:
        print("\n" + "=" * 70)
...
def run_multiple_experiments(n_runs: int = 20, verbose: bool = False) -> Dict:
    ...
    m_empty_means = []
    optimal_ks = []
    fod_detections = []  # True if correctly detected incomplete FOD
    correct_k_detections = []  # True if optimal k matches actual classes
    
    # Expected values
    actual_classes = 3  # Iris has 3 classes
    
    for i in tqdm(range(n_runs), desc="Running experiments"):
        # Use different random seed for each run (None means true random)
        results = example_iris_incomplete_fod(random_seed=None, verbose=verbose)
        
        m_empty_means.append(results['m_empty_mean'])
        
        # Check FOD detection
        fod_incomplete = not results['fod_is_complete']
        fod_detections.append(fod_incomplete)  # Should detect incomplete FOD
        
        # Check optimal k
        optimal_k = results.get('optimal_k', 2)  # Default to known classes if FOD complete
        optimal_ks.append(optimal_k)
        correct_k_detections.append(optimal_k == actual_classes)
    
    # Calculate statistics
    m_empty_means = np.array(m_empty_means)
    optimal_ks = np.array(optimal_ks)
    
    stats = {
        'm_empty_mean': {
            'mean': np.mean(m_empty_means),
            'std': np.std(m_empty_means),
            'min': np.min(m_empty_means),
            'max': np.max(m_empty_means)
        },
        'optimal_k': {
            'mean': np.mean(optimal_ks),
            'std': np.std(optimal_ks),
            'values': optimal_ks.tolist(),
            'mode': int(np.bincount(optimal_ks.astype(int)).argmax())
        },
        'fod_detection': {
            'success_rate': np.mean(fod_detections) * 100,
            'n_correct': np.sum(fod_detections),
            'n_total': n_runs
        },
        'k_detection': {
            'success_rate': np.mean(correct_k_detections) * 100,
            'n_correct': np.sum(correct_k_detections),
            'n_total': n_runs
        },
        'n_runs': n_runs
    }
    ...
    print(f"{'=' * 70}")
    print(f"数据集: Iris")
    print(f"  m̄(∅): {stats['m_empty_mean']['mean']:.4f} ± {stats['m_empty_mean']['std']:.4f}")
    print(f"  Optimal k: {stats['optimal_k']['mean']:.1f} ± {stats['optimal_k']['std']:.2f} (成功率: {stats['k_detection']['success_rate']:.1f}%)")
    print(f"  FOD 检测正确率: {stats['fod_detection']['success_rate']:.1f}%")
    print(f"{'=' * 70}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--multi':
        ...
        # Run single experiment with default (random) seed
